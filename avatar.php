...
Version: 0.1.4
*/
require( dirname( __FILE__ ) . '/templatetags.php' );
...
class Avatar {
	...
	public $dir_transient;
	public $url_transient;
	...
	function upload_dir( $directory = false, $user_id = false ) {
		...
		$path  = $this->path . '/avatars/' . $user_id;
		$newbdir = $path;
		...
		$newurl  = $this->url . '/avatars/' . $user_id;
		$newburl = $newurl;
	...
	function cropper_inline_js() {
		$image = apply_filters( 'inline_cropper_image', getimagesize( $this->path . get_transient( $this->dir_transient ) ) );
		$aspect_ratio = 1;
	...
	function handle_crop( $args = '' ) {
		...
		$original_file = $this->path . $original_file;
		...
		if ( ! $item_id ) {
			...
			$avatar_folder_dir = apply_filters( 'avatar_folder_dir', $this->path . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
		}
	...
	function avatar_filter( $avatar, $user, $size, $default, $alt ) {
		...
		} elseif ( is_string( $user ) && ( $user_by_email = get_user_by_email( $user ) ) ) {
			$id = $user_by_email->ID;
		}
	...
	function fetch( $args = '' ) {
		...
		$avatar_folder_url = apply_filters( 'avatar_folder_url', $this->url . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
		$avatar_folder_dir = apply_filters( 'avatar_folder_dir', $this->path . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
...
add_action( 'plugins_loaded', array( 'Avatar', 'get_instance' ) );